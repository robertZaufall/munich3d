...
On-demand models are generated by `website/server.mjs`, which launches the two
root pipeline scripts in sequence. They are runtime source files and must not
be removed as cleanup. Results are cached below the Git-ignored
`website/.runtime/models/` directory and merged into the chooser through
`/api/models`. Runtime bundles retain only the GLB, source-mesh JSON and
metadata JSON; discard the extractor's reproducible OBJ and README. Runtime
models may be deleted through the local API and UI, but permanent bundles in
`website/public/model/` must never be exposed through that delete path.
...
- Keep the generation server bound to loopback. Never accept a client-supplied
  output path or expose the extraction endpoint publicly by default.
